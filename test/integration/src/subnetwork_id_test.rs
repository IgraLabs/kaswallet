...
use kaswallet_daemon::transaction_generator::TX_VERSION_TOCCATA;
...
/// IGRA user-lane subnetwork id per KIP-21: 4-byte namespace `97 b1 00 00`
/// followed by 16 zero bytes. Any future subnetwork test should share this
/// canonical value rather than redeclaring its own opaque hex literal.
pub const IGRA_LANE_ID_HEX: &str = "97b1000000000000000000000000000000000000";

/// All-zero subnetwork id resolved as the native subnetwork.
const NATIVE_SUBNETWORK_ID_HEX: &str = "0000000000000000000000000000000000000000";

#[rstest]
#[tokio::test]
pub async fn test_send_uses_configured_non_native_subnetwork_id() {
    init_log_for_tests();
    let expected_subnetwork =
        SubnetworkId::from_str(IGRA_LANE_ID_HEX).expect("test constant is valid hex");
    let expected_version = TX_VERSION_TOCCATA;

    run_send_assertions(IGRA_LANE_ID_HEX, expected_subnetwork, expected_version).await;
}
...
pub async fn test_send_with_explicit_native_subnetwork_id_uses_tx_version_zero() {
    init_log_for_tests();
    run_send_assertions(NATIVE_SUBNETWORK_ID_HEX, SUBNETWORK_ID_NATIVE, TX_VERSION).await;
}
